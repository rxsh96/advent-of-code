...
const password_list = passwords.split("\n");
 ...
 */
const password_philosophy = (database) => {
  let good_passwords = 0;
  for (let i = 0; i < database.length; i++) {
    const policy_password = database[i].split(": ");
    const numbers_letter = policy_password[0].split(" ");
    const min_max = numbers_letter[0].split("-");
    const occurrences = policy_password[1].split(numbers_letter[1]).length - 1;
    if (
      policy_password[1].includes(numbers_letter[1]) &&
      occurrences >= Number(min_max[0]) &&
      occurrences <= Number(min_max[1])
    ) {
      good_passwords++;
    }
  }
...
console.log(password_philosophy(password_list));
